...
// Inline brand icons. Using filled-in SVG paths from simple-icons.org
// (CC0) so we avoid adding react-icons (~50KB) just for three glyphs.
// Each icon sits in a 24-size viewBox; we render at 20×20 in a rounded
// tile with the brand's official color so the row has visual weight
// matching IntegrationsPanel's Google logo.
function WhatsappIcon() {
  return (
    <span
      aria-hidden
      className="inline-flex items-center justify-center w-7 h-7 rounded-md bg-[#25D366] text-white shrink-0"
    >
...
// Raw Telegram path — reused by both the tiled brand-mark (row icon)
// and the bare inline glyph (inside the Quick-Connect button, where
// the button's own background provides the colored surround).
const TELEGRAM_SVG_PATH =
...
function TelegramIcon() {
  return (
    <span
      aria-hidden
      className="inline-flex items-center justify-center w-7 h-7 rounded-md bg-[#229ED9] text-white shrink-0"
    >
      <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" aria-hidden>
        <path d={TELEGRAM_SVG_PATH} />
      </svg>
    </span>
  )
}

// Bare glyph (no tile) — for use inside a button where the button
// provides its own surround color. Inherits currentColor from the
// button's text color so it tints correctly in both light + dark.
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <span>{t({ he: 'פתח בטלגרם', en: 'Open in Telegram' })}</span>
      </a>
